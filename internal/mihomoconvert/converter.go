// Adapted from the bundled Mihomo common/convert (GPL-3.0); see NOTICE.md.
package convert
	...
	for _, line := range arr {
		...
		case "trojan":
			...
			case "ws":
				...
				// No synthetic User-Agent is needed when importing a node.
		...
		case "ss":
			...
			if port == "" {
				dcBuf, err := TryDecodeBase64(urlSS.Host)
				if err != nil {
					continue
				}
			...
			if password, found = urlSS.User.Password(); !found {
				dcBuf, err := TryDecodeBase64(cipherRaw)
				if err != nil {
