(() => {
  const proxyTypes=[['ss','Shadowsocks'],['socks5','SOCKS5'],['http','HTTP / HTTPS'],['vmess','VMess'],['vless','VLESS'],['trojan','Trojan'],['hysteria2','Hysteria2'],['tuic','TUIC']];
  ...
  function ensureModals(){
    ...
    const outboundModal=document.createElement('div');outboundModal.className='modal-backdrop';outboundModal.id='mihomo-outbound-modal';outboundModal.innerHTML=`<section class="modal-panel mihomo-modal" role="dialog" aria-modal="true" aria-labelledby="mihomo-outbound-title"><header class="modal-titlebar"><span id="mihomo-outbound-title">Add Outbound</span><button type="button" class="modal-close" data-mihomo-close>${AI('close')}</button></header><form id="mihomo-outbound-form" novalidate><div class="modal-content"><div class="mihomo-editor-tabs" role="tablist" aria-label="Outbound input"><button type="button" role="tab" id="outbound-form-tab" aria-controls="outbound-form-panel" aria-selected="true" data-outbound-mode="form">Form</button><button type="button" role="tab" id="outbound-yaml-tab" aria-controls="outbound-yaml-panel" aria-selected="false" data-outbound-mode="yaml">YAML</button><span class="ink-bar"></span></div><div id="outbound-form-panel" role="tabpanel" aria-labelledby="outbound-form-tab"><div class="mihomo-form-note" data-i18n-html="mh.formNote">Proxy Node 会写入 <code>proxies</code>；Proxy Group 会写入 <code>proxy-groups</code>。两者都可作为 Routing Rule 的目标。</div><div class="mihomo-form-grid">
      ...
      <label class="mihomo-field"><span>Type</span><select name="type"></select></label><label class="mihomo-field" data-kind="proxy"><span>Server</span><input name="server" placeholder="example.com"></label><label class="mihomo-field" data-kind="proxy"><span>Port</span><input name="port" type="number" min="1" max="65535" placeholder="443"></label>
      <label class="mihomo-field" data-kind="proxy" data-types="socks5,http"><span>Username</span><input name="username" autocomplete="off"></label><label class="mihomo-field" data-kind="proxy" data-types="ss,socks5,http,trojan,hysteria2,tuic"><span>Password</span><input name="password" autocomplete="new-password"></label>
  ...
  function syncOutboundForm(){
    ...
    form.elements.server.required=kind==='proxy';form.elements.port.required=kind==='proxy';form.elements.proxies.required=kind==='group';
  ...
  function renderBasicsTags(name){
    ...
    box.innerHTML=(draft.basics[name]||[]).map((value,index)=>'<span class="basics-tag">'+esc(value)+'<button type="button" data-remove-tag="'+index+'" aria-label="'+esc(muiT('mh.removeTag',{value:value}))+'">'+AI('close')+'</button></span>').join('');
  ...
  function ensureWarpModal(){
    ...
    modal.innerHTML=`<section class="modal-panel warp-modal" role="dialog" aria-modal="true" aria-labelledby="warp-title">
      <header class="modal-titlebar"><span id="warp-title">Cloudflare WARP</span><button type="button" class="modal-close" id="warp-close" aria-label="Close" data-i18n-aria="common.close">${AI('close')}</button></header>
