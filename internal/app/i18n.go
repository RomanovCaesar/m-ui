...
var messagePatternsEN = []struct {
	...
	{regexp.MustCompile(`^VPNGate 网卡序号 (\d+) 已被 Outbound (.+) 占用$`), "VPNGate NIC index $1 is already used by outbound $2"},
	{regexp.MustCompile(`^0-(\d+) 号网卡序号都已被占用$`), "All VPNGate NIC indices from 0 to $1 are already used"},
	{regexp.MustCompile(`^没有找到 (\S+) 的 VPNGate 节点$`), "No VPNGate node was found for $1"},
	{regexp.MustCompile(`^没有找到 (.+) 的可用节点$`), "No usable node was found for $1"},
	{regexp.MustCompile(`^VPNGate 节点列表返回状态 (\d+)$`), "The VPNGate node list returned status $1"},
	{regexp.MustCompile(`^IPinfo 返回状态 (\d+)$`), "IPinfo returned status $1"},
	{regexp.MustCompile(`^ip-api 返回状态 (\d+)$`), "ip-api returned status $1"},
	...
	{regexp.MustCompile(`^SoftEther 账户 (\S+) 未能建立连接：(.*)$`), "SoftEther account $1 did not establish a connection: $2"},
	{regexp.MustCompile(`^SoftEther 账户 (\S+) 未能建立连接$`), "SoftEther account $1 did not establish a connection"},
	{regexp.MustCompile(`^SoftEther 网卡 (\S+) 没有 m-ui 所有权记录，拒绝接管$`), "SoftEther adapter $1 has no m-ui ownership record; refusing to take it over"},
	{regexp.MustCompile(`^SoftEther 账户 (\S+) 没有 m-ui 所有权记录，拒绝接管$`), "SoftEther account $1 has no m-ui ownership record; refusing to take it over"},
	{regexp.MustCompile(`^SoftEther 账户 (\S+) 已存在但网卡所有权记录缺失，拒绝接管$`), "SoftEther account $1 exists without an adapter ownership record; refusing to take it over"},
	{regexp.MustCompile(`^策略路由优先级 (\d+) 已被其他配置占用：(.+)$`), "Policy-rule priority $1 is owned by another configuration: $2"},
	{regexp.MustCompile(`^压缩包内存在异常权限：(.+)$`), "The archive contains unsafe permissions: $1"},
	{regexp.MustCompile(`^编译结果 (\S+) 超过大小限制$`), "Build output $1 exceeds the size limit"},
...
var messagesEN = map[string]string{
	...
	"IPinfo Token 不能包含空白字符，且不能超过 128 个字符":                             "The IPinfo Token cannot contain whitespace or exceed 128 characters",
	...
	"备份中的 IPinfo Token 无效":          "The IPinfo Token in the backup is invalid",
	...
	// —— VPNGate ——
	"VPNGate 只支持 Linux 服务器":                          "VPNGate is supported only on Linux servers",
	"VPNGate 已经安装":                                   "VPNGate is already installed",
	"VPNGate 安装正在进行中":                                "VPNGate installation is already running",
	"VPNGate 需要以 root 运行面板":                          "VPNGate requires the panel to run as root",
	"缺少 /dev/net/tun，请先为服务器启用 TUN/TAP":               "Missing /dev/net/tun — enable TUN/TAP for the server first",
	"VPNGate 目录不能是符号链接":                              "The VPNGate directory cannot be a symbolic link",
	"VPNGate 目录被同名文件占用":                              "The VPNGate directory path is occupied by a file",
	"VPNGate 压缩包里没有找到 Makefile":                      "No Makefile was found in the VPNGate archive",
	"压缩包文件数量超过上限":                                    "The archive contains too many files",
	"压缩包解压后体积超过上限":                                   "The extracted archive exceeds the size limit",
	"压缩包是空的":                                         "The archive is empty",
	"VPNGate 节点列表为空":                                 "The VPNGate node list is empty",
	"VPNGate 节点列表超过大小限制":                             "The VPNGate node list exceeds the size limit",
	"不是可查询的公网 IPv4 地址":                               "The address is not a queryable public IPv4 address",
	"IPinfo 查询额度已用尽":                                 "The IPinfo query quota is exhausted",
	"IPinfo 处于冷却中":                                   "IPinfo is cooling down after a quota response",
	"IPinfo 返回了无法解析的内容":                              "IPinfo returned an unreadable response",
	"IPinfo 没有返回 ASN 信息":                             "IPinfo returned no ASN information",
	"ip-api 查询频率超限":                                  "The ip-api rate limit was reached",
	"ip-api 返回了无法解析的内容":                              "ip-api returned an unreadable response",
	"ip-api 没有返回 ASN 信息":                             "ip-api returned no ASN information",
	"请选择 VPNGate 运营商或填写 ASN 名称关键词":                   "Choose a VPNGate ISP or enter an ASN name keyword",
	"VPNGate 运营商关键词不能包含换行，且不能超过 64 个字符":              "The VPNGate ISP keyword cannot contain line breaks or exceed 64 characters",
	"VPNGate 出站不能是策略组":                               "A VPNGate outbound cannot be a proxy group",
	"该 VPNGate 出站还没有保存":                              "This VPNGate outbound has not been saved yet",
	"检测到非 m-ui 管理的 VPN Client 服务，拒绝接管":               "An unmanaged VPN Client service is running; m-ui refused to take it over",
	"m-ui 管理的 VPN Client 服务存在但控制端未就绪":                "The m-ui-managed VPN Client service exists but its control endpoint is not ready",
	"VPNGate 客户端尚未安装":                                "The VPNGate client is not installed",
	"VPNGate 客户端服务正在启动":                              "The VPNGate client service is starting",
	"VPNGate 客户端服务未能就绪":                              "The VPNGate client service did not become ready",
	"VPN Client 服务所有权记录无效":                           "The VPN Client service ownership record is invalid",
	"备份里的 VPNGate 出站需要先在 VPNGate 窗口点击 Install 安装客户端": "VPNGate outbounds in the backup require installing the client from the VPNGate window first",
